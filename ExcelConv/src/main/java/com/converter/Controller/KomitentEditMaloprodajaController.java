...
@SpringComponent
@UIScope
public class KomitentEditMaloprodajaController extends KomitentiEdit{
	...
	@Autowired
	public KomitentEditMaloprodajaController(KomitentService kms, MestoService ms, UstanovaService us) {
		this.kms = kms;
		this.ms = ms;
		this.us = us;
		 ...
		 ok.addClickListener(event->{
	    	   try{
	               ...
	               getFilterMalo().clear();
	               getKomGridMalo().setItems(kms.findAllByTip("KK"));
	    	   }catch(Exception ec){
	    		   ((UI) getWindow().getParent()).removeWindow(getWindow());
